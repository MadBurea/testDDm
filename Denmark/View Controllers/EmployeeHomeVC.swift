...
class EmployeeHomeVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let barButton = UIBarButtonItem(image: #imageLiteral(resourceName: "ic_menu").withRenderingMode(.alwaysOriginal), style: .plain, target: self, action: #selector(EmployeeHomeVC.btnMenuClick))
        self.navigationItem.leftBarButtonItem = barButton
        setupBack()
    }
    @objc func btnMenuClick() {
        if let controller = mainStory.instantiateViewController(withIdentifier: "EmployeeMenuVC") as? EmployeeMenuVC {
